...
;(function (root, factory) {
  ...
  return function (lunr, nodejiebaDictJson) {
    ...
    const tokenizerForStr = (textOrArray, itemIndex = 0) => {
      if (Array.isArray(textOrArray)) {
        return textOrArray
          .map((item, index) => tokenizerForStr(item, index))
          .flat()
      }
      const str = textOrArray.toString().trim().toLowerCase()
      let tokens = []

      str.split(' ').forEach((item) => {
        nodejieba.cut(item, true).forEach((seg) => {
          tokens = [...tokens, ...seg.split(' ')]
        })
        // 全长匹配(TODO: 都加进去索引文件会很大，用 token 搜索感觉没毛病？)
        // tokens = [...tokens, item]
      })
      ...
      return tokens.map((token) => {
        if (isLunr2) {
          const start = str.indexOf(token, fromIndex)

          const tokenMetadata = {}
          tokenMetadata.position = [start, start + token.length]
          tokenMetadata.index = itemIndex
